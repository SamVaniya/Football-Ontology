...
def load_file():
    ...
    if file_path:
        try:
            ...
            display_classes(ontology_graph)
...
def display_classes(graph):
    ...
    for row in results:
        ...
        parent_name = parent.split('#')[-1] if parent and '#' in parent else (parent.split('/')[-1] if parent else None)
        if class_name not in class_hierarchy:
            class_hierarchy[class_name] = []
        if parent_name:
            class_hierarchy[class_name].append(parent_name)
    
    text_output.insert(tk.END, "Classes and their parent classes:\n")
    for cls, parents in class_hierarchy.items():
        parents_str = ', '.join(parents) if parents else 'None'
        text_output.insert(tk.END, f"Class: {cls}, Parent Class: {parents_str}\n")
